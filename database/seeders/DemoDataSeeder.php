...
class DemoDataSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $transitions = [
            ['received', 'verification', ['staff', 'admin', 'super_admin', 'collector'], false],
            ['verification', 'categorised', ['staff', 'admin', 'super_admin', 'collector'], false],
            ...
            ['resolved', 'reopened', ['citizen', 'admin', 'super_admin'], true],
            ['rejected', 'reopened', ['admin', 'super_admin'], true],
            ['reopened', 'verification', ['staff', 'admin', 'super_admin'], false],
        ];
        ...
        $mentorAgri = User::updateOrCreate(
            ...
            [
                ...
                'taluka_id' => $garbadaTaluka?->id,
                'village_id' => $gangardiVillage?->id,
                'address' => 'Faliya Road, Near Krishi Vigyan Kendra',
                'pincode' => '389190',
                'ration_card_no' => 'GJ-20-GRB-481902',
        ...
        $citizens = [];
        foreach ($citizensData as $index => $cData) {
            $rawName = explode(' (', $cData['name'])[0];
            ...
            $citizen = User::updateOrCreate(
                ...
                [
                    ...
                    'last_name' => $lastName,
                    'email' => $cData['phone'].'@ggvt.org',
                    'password' => $defaultPassword,
                    'gender' => $cData['gender'],
                    'age' => $cData['age'],
                    'date_of_birth' => now()->subYears($cData['age'])->format('Y-m-d'),
                    'blood_group' => ['O+', 'B+', 'A+', 'AB+'][$index % 4],
                    'district_id' => $cData['district_id'],
                    'taluka_id' => $cData['taluka_id'],
                    'village_id' => $cData['village_id'],
                    'address' => 'Ward No. '.(($index % 5) + 1).', Near Gram Panchayat',
                    'pincode' => '396'.str_pad((string) (400 + ($index * 12)), 3, '0', STR_PAD_LEFT),
                    'ration_card_no' => 'GJ-24-CKL-'.(882900 + $index),
                    'blood_donor_active' => true,
                    'sms_alerts_active' => true,
        ...
        $settings = [
            ...
            ['key' => 'app_name_short_en', 'group' => 'general', 'value' => 'THH'],
            ['key' => 'app_name_short_gu', 'group' => 'general', 'value' => 'ટીએચએચ'],
